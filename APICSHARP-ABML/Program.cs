...
// Hubs y servicios de SignalR para tiempo real
...
// Registra SignalR. Ya está incluido en el SDK de ASP.NET Core — no requiere NuGet extra.
// AddSignalR() agrega el protocolo JSON de hubs, el gestor de conexiones y el transporte WebSocket.
builder.Services.AddSignalR();

// Singleton que persiste la pregunta activa en memoria durante el ciclo de vida de la app.
// Permite que clientes React que se reconectan recuperen el estado actual.
builder.Services.AddSingleton<IEstadoPreguntaActual, EstadoPreguntaActual>();
...
{
    // Política para los endpoints REST normales: sin restricción de origen.
    ...
    // Política dedicada para el Hub de SignalR.
    //
    // POR QUÉ NO se puede reutilizar "PermitirTodo" con SignalR:
    //   SignalR necesita AllowCredentials() para que el cliente pueda enviar
    //   cookies y headers de autenticación durante el handshake WebSocket.
    //   Pero AllowCredentials() es INCOMPATIBLE con AllowAnyOrigin() por seguridad
    //   (el navegador rechazaría la respuesta con error CORS).
    //   La solución es especificar orígenes explícitos en esta política.
    //
    // Agrega aquí todos los orígenes desde donde se abrirá la web React:
    opts.AddPolicy("PermitirSignalR", politica => politica
        .WithOrigins(
            "http://localhost:3000",
            "http://localhost:5173",
            "http://localhost:4173",
            "https://preguntas-sigma.vercel.app"
        )
        .AllowAnyHeader()
        .AllowAnyMethod()
        .AllowCredentials()
...
// UseHttpsRedirection() desactivado para pruebas locales con Postman/browser HTTP.
// Reactivar en producción: app.UseHttpsRedirection();
...
// Mapea el Hub de SignalR en la ruta /preguntaHub.
// DEBE ir antes de app.Run() para que el endpoint quede registrado.
// .RequireCors("PermitirSignalR") sobreescribe la política global "PermitirTodo"
// SOLO para este endpoint, aplicando la política que admite AllowCredentials().
app.MapHub<PreguntaHub>("/preguntaHub").RequireCors("PermitirSignalR");
