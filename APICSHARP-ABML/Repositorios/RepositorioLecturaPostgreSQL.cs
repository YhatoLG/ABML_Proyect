...
namespace ApiGenericaCsharp.Repositorios
{
    ...
    public sealed class RepositorioLecturaPostgreSQL : IRepositorioLecturaTabla
    {
            ...
            string? camposEncriptar = null)
        {
            ...
            if (datos == null || !datos.Any())
                throw new ArgumentException("Los datos no pueden estar vacíos.", nameof(datos));
//TODO: Implement the logic for updating records in the PostgreSQL database.
            string esquemaFinal = string.IsNullOrWhiteSpace(esquema) ? "public" : esquema.Trim();
            // ACTUALIZADO: soporta PK compuesta recibiendo nombres y valores en CSV.
            var nombresClave = nombreClave
                .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
                .ToArray();
            var valoresClave = valorClave
                .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
                .ToArray();

            if (nombresClave.Length == 0)
                throw new ArgumentException("No se proporcionaron columnas clave válidas.", nameof(nombreClave));

            if (nombresClave.Length != valoresClave.Length)
                throw new ArgumentException("La cantidad de columnas clave y valores de clave no coincide.", nameof(valorClave));
...
//TODO: Implement the logic for updating records in the PostgreSQL database.
            try
            {
                ...
                // ACTUALIZADO: WHERE dinamico con todas las PK (clave1 AND clave2 ...).
                var clausulaWhere = string.Join(" AND ", nombresClave.Select((clave, index) => $"\"{clave}\" = @valorClave{index}"));
                string sql = $"UPDATE \"{esquemaFinal}\".\"{nombreTabla}\" SET {clausulaSet} WHERE {clausulaWhere}";
                ...
                for (int i = 0; i < nombresClave.Length; i++)
                {
                    var tipoColumnaClave = await DetectarTipoColumnaAsync(nombreTabla, esquemaFinal, nombresClave[i]);
                    object valorClaveConvertido = ConvertirValor(valoresClave[i], tipoColumnaClave);
                    string nombreParametro = $"valorClave{i}";

                    if (tipoColumnaClave.HasValue)
                    {
                        var parametro = new NpgsqlParameter(nombreParametro, tipoColumnaClave.Value)
                        {
                    ...
                    {
                        comando.Parameters.AddWithValue(nombreParametro, valoresClave[i]);
                    }
                }
            ...
            string valorClave)
        {
            ...
            if (string.IsNullOrWhiteSpace(valorClave))
                throw new ArgumentException("El valor de la clave no puede estar vacío.", nameof(valorClave));
//TODO: Implement the logic for deleting records from the PostgreSQL database.
            string esquemaFinal = string.IsNullOrWhiteSpace(esquema) ? "public" : esquema.Trim();
            // ACTUALIZADO: soporta PK compuesta recibiendo nombres y valores en CSV.
            var nombresClave = nombreClave
                .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            ...
            try
            {
                ...
                // ACTUALIZADO: WHERE dinamico con todas las PK (clave1 AND clave2 ...).
                var clausulaWhere = string.Join(" AND ", nombresClave.Select((clave, index) => $"\"{clave}\" = @valorClave{index}"));
                string sql = $"DELETE FROM \"{esquemaFinal}\".\"{nombreTabla}\" WHERE {clausulaWhere}";
